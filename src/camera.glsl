...
ViewRayContext vrc_from_uv(daxa_BufferPtr(GpuInput) gpu_input, vec2 uv) {
    ...
    res.ray_dir_vs_h = deref(gpu_input).cam.sample_to_view * res.ray_dir_cs;
    res.ray_dir_ws_h = deref(gpu_input).cam.view_to_world * res.ray_dir_vs_h;
    res.ray_origin_cs = vec4(uv_to_cs(uv), 1.0, 1.0);
    res.ray_origin_vs_h = deref(gpu_input).cam.sample_to_view * res.ray_origin_cs;
...
ViewRayContext unjittered_vrc_from_uv(daxa_BufferPtr(GpuInput) gpu_input, vec2 uv) {
    ViewRayContext res;
    res.ray_dir_cs = vec4(uv_to_cs(uv), 0.0, 1.0);
    res.ray_dir_vs_h = deref(gpu_input).cam.clip_to_view * res.ray_dir_cs;
    res.ray_dir_ws_h = deref(gpu_input).cam.view_to_world * res.ray_dir_vs_h;
    res.ray_origin_cs = vec4(uv_to_cs(uv), 1.0, 1.0);
    res.ray_origin_vs_h = deref(gpu_input).cam.clip_to_view * res.ray_origin_cs;
    res.ray_origin_ws_h = deref(gpu_input).cam.view_to_world * res.ray_origin_vs_h;
    return res;
}
ViewRayContext vrc_from_uv_and_depth(daxa_BufferPtr(GpuInput) gpu_input, vec2 uv, float depth) {
    ...
    res.ray_hit_vs_h = deref(gpu_input).cam.sample_to_view * res.ray_hit_cs;
