...
struct DrawInput {
    daxa::TaskImageView visbuffer64;
    daxa::TaskImageView debug_overdraw;
    daxa::TaskImageView hiz;
    daxa::TaskBufferView indirect_infos;
    daxa::TaskBufferView chunks;
    daxa::TaskBufferView brick_data;
    daxa::TaskBufferView brick_instance_allocator;
    daxa::TaskBufferView brick_meshlet_allocator;
    daxa::TaskBufferView brick_meshlet_metadata;
...
void draw_visbuffer(GpuContext &gpu_context, daxa::TaskGraph &task_graph, DrawInput const &input) {
    ...
    gpu_context.add(RasterTask<DrawVisbuffer::Task, DrawVisbufferPush, uint32_t>{
        ...
        .views = std::array{
            daxa::attachment_view(DrawVisbuffer::AT.visbuffer64, input.visbuffer64),
            daxa::attachment_view(DrawVisbuffer::AT.gpu_input, gpu_context.task_input_buffer),
            daxa::attachment_view(DrawVisbuffer::AT.chunks, input.chunks),
            daxa::attachment_view(DrawVisbuffer::AT.brick_data, input.brick_data),
            daxa::attachment_view(DrawVisbuffer::AT.brick_instance_allocator, input.brick_instance_allocator),
            daxa::attachment_view(DrawVisbuffer::AT.meshlet_allocator, input.brick_meshlet_allocator),
            daxa::attachment_view(DrawVisbuffer::AT.meshlet_metadata, input.brick_meshlet_metadata),
#if ENABLE_DEBUG_VIS
            daxa::attachment_view(DrawVisbuffer::AT.debug_overdraw, input.debug_overdraw),
#endif
            daxa::attachment_view(DrawVisbuffer::AT.indirect_info, input.indirect_infos),
...
void compute_rasterize(GpuContext &gpu_context, daxa::TaskGraph &task_graph, DrawInput const &input) {
    ...
    gpu_context.add(ComputeTask<ComputeRasterize::Task, ComputeRasterizePush, uint32_t>{
        ...
        .extra_defines = extra_defines,
        .views = std::array{
            daxa::attachment_view(ComputeRasterize::AT.visbuffer64, input.visbuffer64),
            daxa::attachment_view(ComputeRasterize::AT.gpu_input, gpu_context.task_input_buffer),
            daxa::attachment_view(ComputeRasterize::AT.chunks, input.chunks),
            daxa::attachment_view(ComputeRasterize::AT.brick_data, input.brick_data),
            daxa::attachment_view(ComputeRasterize::AT.brick_instance_allocator, input.brick_instance_allocator),
            daxa::attachment_view(ComputeRasterize::AT.meshlet_allocator, input.brick_meshlet_allocator),
            daxa::attachment_view(ComputeRasterize::AT.meshlet_metadata, input.brick_meshlet_metadata),
            daxa::attachment_view(ComputeRasterize::AT.indirect_info, input.indirect_infos),
#if ENABLE_DEBUG_VIS
            daxa::attachment_view(ComputeRasterize::AT.debug_overdraw, input.debug_overdraw),
#endif
...
namespace renderer {
    struct VoxelRasterizer {
        TemporalBuffer brick_meshlet_allocator;
        TemporalBuffer brick_meshlet_metadata;
        TemporalBuffer brick_instance_allocator;
        TemporalBuffer brick_visibility_bits;
        TemporalBuffer visible_brick_instance_allocator;
    };

    void on_resize(VoxelRasterizer *self, GpuContext &gpu_context, int size_x, int size_y) {
        gpu_context.remove_temporal_buffer(self->visible_brick_instance_allocator);
        self->visible_brick_instance_allocator = gpu_context.find_or_add_temporal_buffer({
        ...
        temp_task_graph.use_persistent_buffer(self->visible_brick_instance_allocator.task_resource);
        task_fill_buffer(temp_task_graph, self->visible_brick_instance_allocator.task_resource, uint32_t{0});
    ...
    void update(VoxelRasterizer *self, GpuContext &gpu_context, daxa::TaskBuffer &task_chunks, daxa::TaskBuffer &task_brick_data) {
        ...
        temp_task_graph.use_persistent_buffer(self->brick_meshlet_allocator);
        temp_task_graph.use_persistent_buffer(self->brick_instance_allocator);
        temp_task_graph.use_persistent_buffer(self->visible_brick_instance_allocator);
        ...
        gpu_context.add(ComputeTask<SetIndirectInfos::Task, SetIndirectInfosPush, NoTaskInfo>{
            ...
            .views = std::array{
                ...
                daxa::attachment_view(SetIndirectInfos::AT.brick_instance_allocator, self->brick_instance_allocator),
                daxa::attachment_view(SetIndirectInfos::AT.meshlet_allocator, self->brick_meshlet_allocator),
        ...
        gpu_context.add(ComputeTask<ClearDrawFlags::Task, ClearDrawFlagsPush, NoTaskInfo>{
            ...
            .views = std::array{
                ...
                daxa::attachment_view(ClearDrawFlags::AT.brick_instance_allocator, self->brick_instance_allocator),
        ...
        task_fill_buffer(temp_task_graph, self->brick_meshlet_allocator, daxa_u32vec2{});
        task_fill_buffer(temp_task_graph, self->brick_instance_allocator, uint32_t{0});
        task_fill_buffer(temp_task_graph, self->visible_brick_instance_allocator, uint32_t{0});
    ...
    auto render(VoxelRasterizer *self, GpuContext &gpu_context, daxa::TaskGraph &task_graph, daxa::TaskBufferView task_chunks, daxa::TaskBufferView task_brick_data, uint32_t &chunk_n, bool &draw_from_observer) -> std::array<daxa::TaskImageView, 4> {
        ...
        self->brick_meshlet_metadata = gpu_context.find_or_add_temporal_buffer({
            // + 1 for the state at index 0
            .size = sizeof(VoxelMeshletMetadata) * (MAX_MESHLET_COUNT + 1),
            .name = "brick_meshlet_metadata",
        });
        self->brick_instance_allocator = gpu_context.find_or_add_temporal_buffer({
            // + 1 for the state at index 0
            .size = sizeof(BrickInstance) * (MAX_BRICK_INSTANCE_COUNT + 1),
            .name = "brick_instance_allocator",
        });
        self->brick_visibility_bits = gpu_context.find_or_add_temporal_buffer({
            // + 1 for the state at index 0
            .size = sizeof(uint32_t) * (MAX_BRICK_INSTANCE_COUNT + 1),
            .name = "brick_visibility_bits",
        });

        task_graph.use_persistent_buffer(self->brick_meshlet_allocator);
        task_graph.use_persistent_buffer(self->brick_meshlet_metadata);
        task_graph.use_persistent_buffer(self->brick_instance_allocator);
        task_graph.use_persistent_buffer(self->brick_visibility_bits);
        task_graph.use_persistent_buffer(self->visible_brick_instance_allocator);
        ...
        auto draw_brick_instances = [&, first_draw = true](daxa::TaskBufferView task_brick_instance_allocator, daxa::TaskImageView hiz) mutable {
            if (first_draw) {
                task_fill_buffer(task_graph, self->brick_meshlet_allocator, daxa_u32vec2{});
            }
            ...
            gpu_context.add(ComputeTask<MeshVoxelBricks::Task, MeshVoxelBricksPush, size_t>{
                ...
                .views = std::array{
                    ...
                    daxa::attachment_view(MeshVoxelBricks::AT.meshlet_allocator, self->brick_meshlet_allocator),
                    daxa::attachment_view(MeshVoxelBricks::AT.meshlet_metadata, self->brick_meshlet_metadata),
            ...
            draw_visbuffer(
                ...
                {
                    .visbuffer64 = task_visbuffer64,
#if ENABLE_DEBUG_VIS
                    .debug_overdraw = task_debug_overdraw,
#endif
                    .hiz = hiz,
                    .indirect_infos = task_indirect_infos,
                    .chunks = task_chunks,
                    .brick_data = task_brick_data,
                    .brick_instance_allocator = task_brick_instance_allocator,
                    .brick_meshlet_allocator = self->brick_meshlet_allocator,
                    .brick_meshlet_metadata = self->brick_meshlet_metadata,
            ...
            if (draw_from_observer) {
                draw_visbuffer(
                    gpu_context, task_graph,
                    {
                        .visbuffer64 = task_observer_visbuffer64,
#if ENABLE_DEBUG_VIS
                        .debug_overdraw = task_debug_overdraw,
#endif
                        .hiz = hiz,
        ...
        draw_brick_instances(self->brick_instance_allocator, daxa::NullTaskImage);
        ...
        gpu_context.add(ComputeTask<AllocateBrickInstances::Task, AllocateBrickInstancesPush, uint32_t const *>{
            ...
            .views = std::array{
                ...
                daxa::attachment_view(AllocateBrickInstances::AT.brick_instance_allocator, self->brick_instance_allocator),
        ...
        draw_brick_instances(self->brick_instance_allocator, hiz);
        ...
        task_fill_buffer(task_graph, self->visible_brick_instance_allocator, uint32_t{0});
        clear_task_buffers(task_graph, std::array<daxa::TaskBufferView, 1>{self->brick_visibility_bits}, std::array{daxa::BufferClearInfo{.size = sizeof(uint32_t) * (MAX_BRICK_INSTANCE_COUNT + 1)}});
        gpu_context.add(ComputeTask<AnalyzeVisbuffer::Task, AnalyzeVisbufferPush, NoTaskInfo>{
            ...
            .views = std::array{
                ...
                daxa::attachment_view(AnalyzeVisbuffer::AT.brick_instance_allocator, self->brick_instance_allocator),
                daxa::attachment_view(AnalyzeVisbuffer::AT.meshlet_allocator, self->brick_meshlet_allocator),
                daxa::attachment_view(AnalyzeVisbuffer::AT.meshlet_metadata, self->brick_meshlet_metadata),
                daxa::attachment_view(AnalyzeVisbuffer::AT.brick_visibility_bits, self->brick_visibility_bits),
                daxa::attachment_view(AnalyzeVisbuffer::AT.visible_brick_instance_allocator, self->visible_brick_instance_allocator),
        ...
        gpu_context.add(ComputeTask<ShadeVisbuffer::Task, ShadeVisbufferPush, NoTaskInfo>{
            ...
            .views = std::array{
                ...
                daxa::attachment_view(ShadeVisbuffer::AT.brick_instance_allocator, self->brick_instance_allocator),
                daxa::attachment_view(ShadeVisbuffer::AT.meshlet_allocator, self->brick_meshlet_allocator),
                daxa::attachment_view(ShadeVisbuffer::AT.meshlet_metadata, self->brick_meshlet_metadata),
        ...
        task_graph.add_task({
            .attachments = {
                daxa::inl_attachment(daxa::TaskBufferAccess::TRANSFER_READ, self->visible_brick_instance_allocator),
                daxa::inl_attachment(daxa::TaskBufferAccess::TRANSFER_WRITE, self->brick_instance_allocator),
            },
