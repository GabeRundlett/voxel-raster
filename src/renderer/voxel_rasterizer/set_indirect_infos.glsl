...
void main() {
    ...
    uint prev_hw_meshlet_count = deref(push.uses.indirect_info[3]).x;
    uint prev_hw_meshlet_offset = deref(push.uses.indirect_info[3]).offset;
    ...
    // compute raster info
    deref(push.uses.indirect_info[1]).x = (new_sw_meshlet_count - (prev_sw_meshlet_offset + prev_sw_meshlet_count) + 3) / 4;
    ...
    deref(push.uses.indirect_info[2]).x = new_sw_meshlet_count;
    deref(push.uses.indirect_info[2]).y = 1;
    deref(push.uses.indirect_info[2]).z = 1;
    deref(push.uses.indirect_info[2]).offset = 0;

    deref(push.uses.indirect_info[3]).x = new_hw_meshlet_count;
    deref(push.uses.indirect_info[3]).y = 1;
    deref(push.uses.indirect_info[3]).z = 1;
    deref(push.uses.indirect_info[3]).offset = 0;

    deref(push.uses.indirect_info[1]).x = (new_sw_meshlet_count + 3) / 4;
    deref(push.uses.indirect_info[1]).y = 1;
    deref(push.uses.indirect_info[1]).z = 1;
