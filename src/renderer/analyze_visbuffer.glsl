...
void write_visible_brick(uint brick_index) {
    uint word_index = brick_index / 32;
    uint in_word_index = brick_index % 32;
    uint visibility_mask = 1 << in_word_index;
    uint prev_bits = atomicOr(deref(push.uses.brick_visibility_bits[word_index]), visibility_bit);
    if ((prev_bits & visibility_mask) != 0) {
        // We're the first thread to set the flag, so we can write ourselves out
        uint visible_brick_index = atomicAdd(deref(push.uses.visible_brick_indices[0]), 1);
        deref(push.uses.visible_brick_indices[visible_brick_index + 1]) = brick_index;
    }
}
...
void main() {
    ...
    uvec4 visbuffer_ids;
    visbuffer_ids[0] = texelFetch(daxa_utexture2D(push.uses.visbuffer), sample_index + ivec2(0, 0), 0);
    visbuffer_ids[1] = texelFetch(daxa_utexture2D(push.uses.visbuffer), sample_index + ivec2(0, 1), 0);
    visbuffer_ids[2] = texelFetch(daxa_utexture2D(push.uses.visbuffer), sample_index + ivec2(1, 0), 0);
    visbuffer_ids[3] = texelFetch(daxa_utexture2D(push.uses.visbuffer), sample_index + ivec2(1, 1), 0);

    uvec4 brick_indices = uvec4(0);
    uint list_mask = 0;
...
#pragma unroll
    for (uint i = 0; i < 4; ++i) {
        ...
        brick_indices[i] = metadata.brick_id;
    }
    ...
    while (subgroupAny(assigned_meshlet_index == ~0) && subgroupAny(list_mask != 0)) {
        ...
        const uint voting_candidate = thread_active ? brick_indices[findLSB(list_mask)] : 0;
...
#pragma unroll
        for (uint i = 0; i < 4; ++i) {
            if (brick_indices[i] == elected_meshlet_instance_index) {
                list_mask &= ~(1u << i);
            }
        }
...
#pragma loop
    while (list_mask != 0) {
        ...
        uint brick_index = brick_indices[lsb];
        list_mask &= ~(1 << lsb);
        write_visible_brick(brick_index);
    }
}
